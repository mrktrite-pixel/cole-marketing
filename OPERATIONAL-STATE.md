...
## PAID PATH ARCHITECTURE (taxchecknow) — SOURCE OF TRUTH FROM CHAT A

The post-purchase delivery surface ("paid path") on taxchecknow.com is
operator-built, signed off, and per Chat A's ORIGINAL-DESIGN-READ-FIRST.md
Part 14 represents the canonical operator-owned reference implementation.
Future cluster sites (Block 7+) will get COLE-built equivalents end-to-end;
taxchecknow's paid path stays as the operator-built proof.

### Mechanism (what actually happens on purchase, observed in codebase)

1. **Calculator finishes** — verdict displays. Per-calculator state writes to:
   - `decision_sessions` row via `POST /api/decision-sessions` (Step 2 fix).
   - `sessionStorage` keys (e.g., `${productId}_tsb_band`, `${productId}_status`,
     `${productId}_tier`, etc.) — keys MUST match `successPromptFields` in the
     product config. **These survive only within the same browser tab.**
2. **Customer clicks Stripe checkout button** — calculator calls
   `POST /api/create-checkout-session` with `decision_session_id` + tier +
   product key. Returns Stripe checkout URL. Customer redirects to Stripe.
3. **Stripe completes payment** — webhook at `/api/stripe/webhook` fires
   `checkout.session.completed`. Webhook:
   - Records purchase in `purchases` table.
   - Calls `generateAndStoreAssessment()` → POSTs to `/api/assess` with
     decision_session inputs → stores result in `assessments` table
     (non-blocking, silent-fail).
   - Queues reminder emails (per `PRODUCT_DEADLINES`).
   - Sends T2 delivery email via `lib/cole-email.ts:sendDeliveryEmail`.
4. **Customer redirected to success page** — `/[country]/check/[product]/success/{plan|assess}?session_id=cs_test_...`. Page reads:
   - `sessionStorage` keys for fast personalised display
   - `/api/get-assessment?session_id=...` for stored assessment_json
   - `/api/get-session?id=...` for Stripe customer name + tier
5. **Customer downloads inline files** — per CLAUDE.md L42: delivery is
   inline via `files` array in product config (8 files: 5 × tier:1 + 3 × tier:2).
   No Google Drive, no `NEXT_PUBLIC_DRIVE`.

### Three protected zones (Session B does NOT touch on taxchecknow)

| Zone | Files | Why protected |
|---|---|---|
| **Success pages** | `app/[country]/check/[product]/success/{plan,assess}/page.tsx` (94 files) | Operator-built, signed off. Customer-facing post-purchase surface. |
| **Checkout route** | `app/api/create-checkout-session/route.ts` | Operator-built. Stripe wire — touching it risks payment-collection breakage. |
| **sessionStorage keys** | `${productId}_${fieldname}` pattern in calculator components | Keys MUST match `successPromptFields` in product config. Renaming or removing breaks success page rendering. |

Adjacent surfaces operator OWNS (also off-limits to Session B):
- `app/api/get-assessment/route.ts` (success page reads from this)
- `app/api/get-session/route.ts` (Stripe session metadata read)
- `app/api/assess/route.ts` (Anthropic-direct assessment generation)
- The 47 `cole/config/*.ts` product configs

### What Session B IS free to work on (taxchecknow)

- `lib/cole-email.ts`, `lib/email-context.ts`, `lib/email-product-copy.ts`,
  `lib/email-types.ts`, `lib/product-deadlines.ts`, `lib/email-templates/`
- `app/api/stripe/webhook/route.ts` routing logic (NOT the assess/checkout
  call sites — those stay)
- `app/api/cron/send-emails/`
- `app/api/leads/route.ts` (save-box T2 + nurture queue)
- `app/api/decision-sessions/` (NEW Step 2 — calculator hydration data plane)
- The save-box hydration `useEffect` block in calculator components
  (Step 2 — purely additive React state, doesn't touch sessionStorage)
- Distribution chain (G5 → H1 → J3, Soverella + lib/distribution-bee.ts)
- Marketing surfaces (`/stories/`, `/questions/`, `/gpt/`, `/llms.txt`)
- All bee runner code in soverella

### Note about webhook's `generateAndStoreAssessment`

The Stripe webhook calls `generateAndStoreAssessment` non-blocking
(`.catch(() => {})`). It POSTs to `/api/assess` with decision_session.inputs
and stores result in `assessments` table.

Per Chat A's confirmation (May 7): the customer-facing assessment is
generated by the SUCCESS PAGE reading sessionStorage and calling Anthropic
API directly from browser. The webhook's `generateAndStoreAssessment` is
NOT the customer-facing path. It may be vestigial OR running in parallel
for archival/Doctor Bee analytics. Customer impact is zero either way.

⚠️ DO NOT REPLICATE THIS PATTERN ON NEW CLUSTER SITES. The browser-direct
Anthropic API call architecture has known failure modes (browser-close
mid-purchase = empty sessionStorage = generic fallback assessment).
Block 7+ cluster sites get COLE-built paid paths that use server-side
generation with DB persistence to avoid this failure mode. This vestigial
webhook code on taxchecknow is taxchecknow-ONLY legacy, not a pattern to
copy.

D-task: clarify whether to clean up `generateAndStoreAssessment` or keep
as archival. Defer to Block 5+ when Doctor Bee work surfaces this naturally.

### For new sites (Block 7+)

COLE builds the full paid path a-to-z including success pages, checkout
wiring, and assessment generation. New sites validate that COLE can do
end-to-end what was operator-built for taxchecknow. Day 10 sprint
deliverable is "COLE re-runs taxchecknow as the validation test."

COLE-built paid paths use server-side assessment generation with database
persistence. The browser-close failure mode present in taxchecknow's
architecture is mitigated in COLE-built sites. Day 10 validation = COLE
rebuilds taxchecknow to demonstrate this mitigated architecture works
end-to-end.

### Reference

- Chat A's `ORIGINAL-DESIGN-READ-FIRST.md` Part 14 (lives in skill folder,
  not in this repo) — full architectural intent
- `cluster-worldwide/taxchecknow/CLAUDE.md` lines 4-7 (delivery field rules)
- `OPERATOR BOUNDARIES — SESSION B SCOPE BY SITE` section above

---

## STAGE-GATE COMMIT PATTERN (locked Day 2)

Every stage gate within a bee or sprint task = a discrete commit + verification.

**Why:** each commit becomes a rollback point. If sub-step N+1 breaks
production, sub-step N is the known-good baseline. Bundling sub-steps into
single commits creates batch failures with no clean bisection.

**Application:**
- Step 2 of save-box β = commit + verify before Step 3
- Each high-risk bee operation gets its own commit
- Low-risk operations (formatting, naming) can batch
- Risk tier per sub-step determines commit granularity

This pattern is foundational to the Day 3 stage-gated bee architecture lock.
Same principle, applied to commit graph.
...
24. **Calculator template heterogeneity surfaced during Step 2 hydration.** At least 2 calculator templates exist in the 47-product taxchecknow codebase: standard pattern (46 calculators — useEffect on showVerdict does decision_sessions POST) and the iso-amt-sniper newer pattern (POST inside handleBracketSelect event handler).

    PREREQUISITE for Day 10 COLE re-run: operator must decide:
    (a) Unify taxchecknow to single template manually before Day 10 (~4-8 hours of manual work)
    (b) Teach COLE to handle both templates during re-run (more bee complexity, more edge cases)

    For NEW cluster sites: COLE generates from a SINGLE canonical template. Heterogeneity is a taxchecknow legacy artifact, not a pattern to replicate.

    D-task: catalog all template variations across 47 calculators before Day 9 (gives operator time to make the prerequisite decision before Day 10 re-run fires).
...
- New canonical section: "PAID PATH ARCHITECTURE (taxchecknow) — SOURCE OF TRUTH FROM CHAT A" — documents observed paid-path mechanism, 3 protected zones, vestigial `generateAndStoreAssessment` framing, do-not-replicate warning for new sites, references Chat A's Part 14
- New canonical section: "STAGE-GATE COMMIT PATTERN (locked Day 2)" — every stage gate = discrete commit + verification; rollback-friendly bisection-friendly commit graph; foundational to Day 3 bee architecture lock
- New discovery #24: calculator template heterogeneity (46 standard + 1 iso-amt-sniper newer) — Day 9 catalog D-task before Day 10 re-run
- New discovery #25: `/api/decision-sessions` route was missing entirely — repaired in Step 2; calculator hydration now functions end-to-end
