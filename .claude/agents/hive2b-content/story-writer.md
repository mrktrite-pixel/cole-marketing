...
Read the file before writing the story or any social copy. Extract the
...
tag, write `[FACT NEEDED: <short description>]` in the output where
...
output with `[FACT NEEDED]` placeholders for every numeric, date,
...
The operator must populate / verify the facts file before the content
can ship. This is the verified-source gate that keeps 920 articles +
their downstream stories, social posts, and emails from spreading
misinformation.

Log to `agent_log` after the run with the `result` field including
`facts_file_used: "knowledge/<file>.md"` and `fact_needed_count: <N>`
(0 if all facts resolved).
